...
WITH t AS 
(
  SELECT id, TO_JSONB(ARRAY[d0,d1,d2,d3,d4,d5,d6,d7,d8,d9]) AS c 
  FROM digits2
),
...
(
  ...
  JOIN t7 
     USING (id) 
  WHERE d069 NOT IN (d6,d9) 
   AND d235 NOT IN (d3,d5) 
) ,

s AS
(
  SELECT id, 
  UNNEST(ARRAY[3,2,1,0]) AS pos, TO_JSONB(STRING_TO_ARRAY(UNNEST(ARRAY[d1,d2,d3,d4]), NULL)) AS encoding 
  FROM digits
)
