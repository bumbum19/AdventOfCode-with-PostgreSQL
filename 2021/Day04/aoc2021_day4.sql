...
(
	...
	GROUP BY GROUPING SETS (
	    (id, board, x_pos),
	    (id, board, y_pos) )
	WINDOW w AS (PARTITION BY board, x_pos, y_pos 
...
(
	SELECT board, id, 
	num * SUM(num) OVER w AS score
	FROM  bingo2 
	JOIN  marker 
		USING(num)    
	GROUP BY board, id, num  
	WINDOW w AS (PARTITION BY board 
		     ORDER BY id DESC  ROWS UNBOUNDED PRECEDING EXCLUDE CURRENT ROW)
)
...
cte3 AS 
(
	SELECT DISTINCT ON (board) board, id 
	FROM cte 
	WHERE  cnt = 5  
	ORDER BY board, id 
)
