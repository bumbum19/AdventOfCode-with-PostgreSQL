...
CREATE SERVER aoc2019 FOREIGN  DATA wrapper file_fdw;


-- Read data

CREATE FOREIGN TABLE aoc2019_day1 (x INT)
SERVER aoc2019 options(filename 'D:\aoc2019.day1.input');

-- Create base table

CREATE  TEMPORARY TABLE rocket
(
	id SERIAL,
	fuel INT
);
...
INSERT INTO rocket(fuel)
SELECT * FROM aoc2019_day1;



-- First Star

SELECT SUM(fuel/3-2) FROM rocket;
...
Fuel itself requires fuel just like a module - take its mass, divide by three, round down, and subtract 2. However, that fuel also requires fuel, and that fuel requires fuel,
and so on. Any mass that would require negative fuel should instead be treated as if it requires zero fuel; the remaining mass, if any, 
...
WITH RECURSIVE cte (fuel, step) AS
( 
    SELECT fuel, 0 FROM rocket
    UNION ALL 
    SELECT GREATEST(fuel/3-2,0), step + 1  
    FROM cte WHERE fuel > 0
)


SELECT SUM(fuel) - SUM(fuel) FILTER (WHERE step = 0) AS fuel_requirement 
FROM cte;
