...
(the cycle number multiplied by the value of the X register) during the 20th cycle and every 40 cycles after that
...
(
	SELECT id,
	2 * COUNT(*) FILTER (WHERE instruction = 'addx' ) OVER w + 
...
( 
	SELECT pos, 
	LEAD(pos) OVER w AS next_pos, val 
	FROM cte
	WINDOW w AS (ORDER BY id)
...
It seems like the X register controls the horizontal position of a sprite. Specifically, the sprite is 3 pixels wide, and the X register sets the horizontal 
position of the middle of that sprite. (In this system, there is no such thing as "vertical position": if the sprite's horizontal position puts its pixels 
...
(
	...
	SELECT t.pos, 
	CASE WHEN CASE WHEN MOD(t.pos,40) = 0 THEN 40 ELSE MOD(t.pos,40) END - COALESCE(val,0) BETWEEN 0 AND 2 
		THEN '#' ELSE '.' END AS pixel,
	NTILE(6) OVER w  AS bucket 
	FROM GENERATE_SERIES(1,240) AS t(pos)
	LEFT JOIN cte2
		ON t.pos - 1 >= cte2.pos 
		AND  t.pos - 1 < cte2.next_pos
	WINDOW w AS (ORDER BY t.pos)
)

SELECT STRING_AGG(pixel,'' ORDER BY pos) AS image 
FROM screen 
GROUP BY bucket 
ORDER BY bucket;
